...
!  Parameters:
!
!    Input, integer  N, the number of nodes and associated 
!    data values.  10 <= N.
!
!    Input, real(8)  X(N), Y(N), Z(N), the coordinates of the nodes.
!
!    Input, real(8)  F(N), the data values at the nodes.
!
!    Input, integer  NQ, the number of data points to be used in
!    the least squares fit for coefficients defining the nodal functions Q(K).  
!    A recommended value is NQ = 17.  9 <= NQ <= MIN ( 40, N-1 ).
!
!    Input, integer  NW, the number of nodes within (and defining) 
!    the radii of influence R(K) which enter into the weights W(K).  For N 
!    sufficiently large, a recommended value is NW = 32.  
!    1 <= NW <= min(40,N-1).
!
!    Input, integer  NR, the number of rows, columns, and planes in
...
!
!    Output, real(8)  xyzmin(3), xyzdel(3), containing 
!    minimum nodal coordinates and cell dim-
!    ensions, respectively.  Refer to store3.
!
!    Output, real(8)  RMAX, square root of the largest element in 
!    RSQ, maximum radius R(K).
!
!    Output, real(8)  RSQ(N) = the squares of the radii r(k)
!    which enter into the weights W(K).
!
!    Output, real(8)  A(9,N), the coefficients for
...
!
  ...
  real(8)  c
  real(8)  dmin
  real(8) , parameter :: dtol = 0.01D+00
  real(8)  f(n)
  real(8)  fk
  integer  i
  integer  ib
  ...
  real(8)  rsold
  real(8)  rsq(n)
  real(8) , parameter :: rtol = 1.0D-05
  real(8)  rws
  real(8)  s
  real(8) , parameter :: sf = 1.0D+00
  real(8)  sum2
  real(8)  t
  real(8)  x(n)
  real(8)  xk
...
!  Parameters:
!
!    Input, real(8)  PX, PY, PZ, the point P at which Q is 
!    to be evaluated.
!
!    Input, integer  N, the number of nodes and data values 
!    defining Q.  10 <= N.
!
!    Input, real(8)  X(N), Y(N), Z(N), F(N), the node coordinates
!    and data values interpolated by Q.
!
!    Input, integer  NR, the number of rows, columns and planes in
!    the cell grid.  Refer to STORE3.  1 <= NR.
!
!    Input, integer  LCELL(NR,NR,NR), nodal indices associated 
!    with cells.  Refer to STORE3.
!
!    Input, integer  LNEXT(N), the next-node indices.  Refer
!    to STORE3.
!
!    Input, real(8)  XYZMIN(3), XYZDEL(3), the minimum nodal
!    coordinates and cell dimensions, respectively.  XYZDEL elements 
!    must be positive.  Refer to STORE3.
!
!    Input, real(8)  RMAX, the square root of the largest 
!    element in RSQ, the maximum radius.
!
!    Input, real(8)  RSQ(N), the squared radii which enter 
!    into the weights defining Q.
!
!    Input, real(8)  A(9,N), the coefficients for the nodal 
!    functions defining Q.
!
!    Output, real(8)  QS3VAL, the function value Q(PX,PY,PZ)
...
!  Parameters:
!
!    Input, real(8)  PX, PY, PZ, the point P at which Q and
!    its partials are to be evaluated.
!
!    Input, integer  N, the number of nodes and data values 
!    defining Q.  10 <= N.
!
!    Input, real(8)  X(N), Y(N), Z(N), F(N), the node coordinates and
...
!
!    Input, real(8)  RMAX, the square root of the largest element
!    in RSQ, the maximum radius.
!
!    Input, real(8)  RSQ(N), the squared radii which enter into
!    the weights defining Q.
!
!    Input, real(8)  A(9,N), the coefficients for the nodal 
!    functions defining Q.
!
!    Output, real(8)  Q, the value of Q at (PX,PY,PZ) unless
!    IER == 1, in which case no values are returned.
!
!    Output, real(8)  QX, QY, QZ, the first partial derivatives of Q at
...
!  Parameters:
!
!    Input, real(8)  PX, PY, PZ, the coordinates of the point P
!    whose nearest unmarked neighbor is to be found.
!
!    Input, real(8)  X(N), Y(N), Z(N), the coordinates of the nodes.
!
!    Input, integer  NR, the number of rows, columns, and planes
!    in the cell grid.  1 <= NR.
!
!    Input, integer  LCELL(NR,NR,NR), nodal indices associated 
!    with cells.
!
!    Input/output, integer  LNEXT(N), next-node indices (or their
!    negatives).
!
!    Input, real(8)  XYZMIN(3), XYZDEL(3), minimum nodal 
!    coordinates and cell dimensions, respectively.  XYZEL elements must 
!    be positive.
!
!    Output, integer  NP, index of the nearest unmarked node to P, 
!    or 0 if all nodes are marked or NR < 1 or an element of XYZDEL is not 
!    positive.  LNEXT(NP) < 0 if NP /= 0.
!
!    Output, real(8)  DSQ, squared euclidean distance between 
...
!  Parameters:
!
!    Input/output, real(8)  A.  On input, the first component of
!    the vector.  On output, overwritten by r = +/-sqrt(a*a + b*b)
!
!    Input/output, real(8)  B.  On input, the second component of 
...
!
!    Output, real(8)  C, c = +/-(a/r)
!
!    Output, real(8)  S, s = +/-(b/r)
!
!  Local parameters:
!
...
!
!    Input, real(8)  C, S, the elements of the Givens rotation.
!    These may be determined by subroutine GIVENS.
!
!    Input/output, real(8)  X(N), Y(N), the vectors to be rotated.
!
  implicit none
...
!  Parameters:
!
!    Input, real(8)  XK, YK, ZK, FK = coordinates and data value
!    at node K (interpolated by q).
!
!    Input, real(8)  XI, YI, ZI, FI = coordinates and data value
!    at node I.
!
!    Input, real(8)  S1, S2 = reciprocals of the scale factors.
!
!    Input, real(8)  R = radius of influence about node K defining the
!    weight.
!
!    Output, real(8)  ROW(10), a row of the augmented
!    regression matrix.
!
!  Local parameters:
!
...
!    if ier /= 0.
!
!    Output, real(8)  XYZMIN(3), the minimum
...
!
!    Output, real(8)  XYZDEL(3), the dimensions
